#target photoshop
// FramerPS version 1.5-19-gc0b2394 (c) 2013 Koen Bok
...
var SaveJPG, SavePNG, checkDocument, console, createFolder, dupLayers, exportImage, fileExists, frameForBounds, getCumulativeHash, getHash, getMetaData, getModificationTime, getXMPData, json, logger, psAlert, readFile, rename, renameLayer, strFrame, writeFile, writeTemplate,
...
rename = function(names, name) {
  ...
  newName = name;
  while (indexOf.call(names, newName) >= 0) {
    newName = name + "-" + c;
    c++;
  }
  return newName;
...
exportImage = function(layerSet, path, imageType) {
  var height, j, layerName, len, newHeight, newWidth, oldHeight, oldWidth, ref1, rootLayerSet, width, x, y;
  if (imageType == null) {
    imageType = "png";
  }
...
main = function() {
  var PREVIOUS, changedCount, count, data, document, err, fakeLayerGroups, fakeLayers, folder, i, jsLayerInfo, jsonLayerInfo, layerGroupNames, layerGroupRenames, layerInfo, len, process, ref, rootLayerGroups, throwError, traverse;
  if (!checkDocument()) {
  ...
  if (indexOf.call(CONFIG.path, "AutoRecover") >= 0) {
  ...
  ref = [CONFIG.path, CONFIG.imagePath];
  ...
  try {
    ...
    err = error;
    console.log("Could not read previous export (for caching), doing full export.");
    data = false;
  }
  PREVIOUS = {};
  ...
  _(fakeLayerGroups).map(function(layerGroup) {
    var invalidChar, invalidChars, j, layerGroupName, layerGroupNameOld, len1;
    layerGroupName = renameLayer(layerGroup.getName());
    layerGroupNameOld = renameLayer(layerGroup.getName());
    invalidChars = ["/", ":"];
    for (j = 0, len1 = invalidChars.length; j < len1; j++) {
      invalidChar = invalidChars[j];
      if (layerGroupName.match(invalidChar)) {
        layerGroupName = layerGroupName.replace(invalidChar, "_");
      }
    }
    if (indexOf.call(layerGroupNames, layerGroupName) >= 0) {
  ...
  traverse = function(layerGroups) {
    return _(layerGroups).map(function(layerGroup) {
      ...
      prefix = (renameLayer(layerGroup.getName())) + ": ";
      count++;
      ...
      imagePath = CONFIG.imagePath + "/" + (renameLayer(layerGroup.getName())) + ".png";
      relativeImagePath = CONFIG.relativeImagePath + "/" + (renameLayer(layerGroup.getName())) + ".png";
      ...
      layerInfo = {
        id: layerGroup.getID(),
        name: renameLayer(layerGroup.getName()),
        layerFrame: null,
        maskFrame: null,
        image: null,
        imageType: null,
  ...
  jsLayerInfo += "window.__imported__[\"" + CONFIG.docName + "@1x/layers.json.js\"] = " + jsonLayerInfo;
