...
function drawGameOver(text) {
  context.clearRect(stage.width / 4, stage.height / 3, stage.width / 2, stage.height / 2);
  context.textAlign = "center";
  context.font = "24px Courier New";
...
var ticker$ = rxjs_1.interval(TICKER_INTERVAL, rxjs_1.animationFrameScheduler).pipe(operators_1.map(function () {
  return {
    time: Date.now(),
    deltaTime: null
  };
}), operators_1.scan(function (prev, current) {
  return {
    time: current.time,
...
var key$ = operators_1.merge(rxjs_1.fromEvent(document, "keydown").pipe(operators_1.map(function (event) {
  return PADDLE_CONTROLS[event.key];
})), rxjs_1.fromEvent(document, "keyup").pipe(operators_1.map(function () {
  return 0;
})));
