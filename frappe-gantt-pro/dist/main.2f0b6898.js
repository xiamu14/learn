...
class Bar {
    ...
    prepare_values() {
        ...
        this.duration = _date_utils2.default.diff(this.task._end, this.task._start, 'hour') / this.gantt.options.step;
    ...
    setup_click_event() {
        _svg_utils.$.on(this.group, 'focus click', e => {
    ...
    show_popup() {
        ...
        const start_date = _date_utils2.default.format(this.task._start, 'MMM D');
        const end_date = _date_utils2.default.format(_date_utils2.default.add(this.task._end, -1, 'second'), 'MMM D');
    ...
    date_changed() {
        ...
        this.gantt.trigger_event('date_change', [this.task, new_start_date, _date_utils2.default.add(new_end_date, -1, 'second')]);
    }
    ...
    compute_start_end_date() {
        ...
        const new_start_date = _date_utils2.default.add(this.gantt.gantt_start, x_in_units * this.gantt.options.step, 'hour');
        const width_in_units = bar.getWidth() / this.gantt.options.column_width;
        const new_end_date = _date_utils2.default.add(new_start_date, width_in_units * this.gantt.options.step, 'hour');
    ...
    compute_x() {
        ...
        const diff = _date_utils2.default.diff(task_start, gantt_start, 'hour');
        let x = diff / step * column_width;

        if (this.gantt.view_is('Month')) {
            const diff = _date_utils2.default.diff(task_start, gantt_start, 'day');
...
},{"./date_utils":16,"./svg_utils":17}],19:[function(require,module,exports) {
'use strict';
...
},{"./svg_utils":17}],20:[function(require,module,exports) {
'use strict';
...
class Gantt {
    ...
    setup_tasks(tasks) {
        ...
        this.tasks = tasks.map((task, i) => {
            ...
            task._start = _date_utils2.default.parse(task.start);
            task._end = _date_utils2.default.parse(task.end);

            // make task invalid if duration too large
            if (_date_utils2.default.diff(task._end, task._start, 'year') > 10) {
                task.end = null;
            }
            ...
            if (!task.start && !task.end) {
                const today = _date_utils2.default.today();
                task._start = today;
                task._end = _date_utils2.default.add(today, 2, 'day');
            }

            if (!task.start && task.end) {
                task._start = _date_utils2.default.add(task._end, -2, 'day');
            }

            if (task.start && !task.end) {
                task._end = _date_utils2.default.add(task._start, 2, 'day');
            }

            // if hours is not set, assume the last day is full day
            // e.g: 2018-09-09 becomes 2018-09-09 23:59:59
            const task_end_values = _date_utils2.default.get_date_values(task._end);
            if (task_end_values.slice(3).every(d => d === 0)) {
                task._end = _date_utils2.default.add(task._end, 24, 'hour');
            }
    ...
    setup_gantt_dates() {
        ...
        this.gantt_start = _date_utils2.default.start_of(this.gantt_start, 'day');
        this.gantt_end = _date_utils2.default.start_of(this.gantt_end, 'day');

        // add date padding on both sides
        if (this.view_is(['Quarter Day', 'Half Day'])) {
            this.gantt_start = _date_utils2.default.add(this.gantt_start, -7, 'day');
            this.gantt_end = _date_utils2.default.add(this.gantt_end, 7, 'day');
        } else if (this.view_is('Month')) {
            this.gantt_start = _date_utils2.default.start_of(this.gantt_start, 'year');
            this.gantt_end = _date_utils2.default.add(this.gantt_end, 1, 'year');
        } else {
            this.gantt_start = _date_utils2.default.add(this.gantt_start, -1, 'month');
            this.gantt_end = _date_utils2.default.add(this.gantt_end, 1, 'month');
        }
    }
    ...
    setup_date_values() {
        ...
        while (cur_date === null || cur_date < this.gantt_end) {
            if (!cur_date) {
                cur_date = _date_utils2.default.clone(this.gantt_start);
            } else {
                cur_date = this.view_is('Month') ? _date_utils2.default.add(cur_date, 1, 'month') : _date_utils2.default.add(cur_date, this.options.step, 'hour');
            }
    ...
    make_grid_rows() {
        const rows_layer = (0, _svg_utils.createSVG)('g', { append_to: this.layers.grid });
        const lines_layer = (0, _svg_utils.createSVG)('g', { append_to: this.layers.grid });
    ...
    make_grid_ticks() {
        ...
        for (let date of this.dates) {
            ...
            if (this.view_is('Month')) {
                tick_x += _date_utils2.default.get_days_in_month(date) * this.options.column_width / 30;
            } else {
    ...
    make_grid_highlights() {
        ...
        if (this.view_is('Day')) {
            const x = _date_utils2.default.diff(_date_utils2.default.today(), this.gantt_start, 'hour') / this.options.step * this.options.column_width;
            const y = 0;
    ...
    make_dates() {
        for (let date of this.get_dates_to_draw()) {
            ...
            if (date.upper_text) {
                const $upper_text = (0, _svg_utils.createSVG)('text', {
                    x: date.upper_x,
                    y: date.upper_y,
    ...
    get_date_info(date, last_date, i) {
        if (!last_date) {
            last_date = _date_utils2.default.add(date, 1, 'year');
        }
        const date_text = {
            'Quarter Day_lower': _date_utils2.default.format(date, 'HH'),
            'Half Day_lower': _date_utils2.default.format(date, 'HH'),
            Day_lower: date.getDate() !== last_date.getDate() ? _date_utils2.default.format(date, 'D') : '',
            Week_lower: date.getMonth() !== last_date.getMonth() ? _date_utils2.default.format(date, 'D MMM') : _date_utils2.default.format(date, 'D'),
            Month_lower: _date_utils2.default.format(date, 'MMMM'),
            'Quarter Day_upper': date.getDate() !== last_date.getDate() ? _date_utils2.default.format(date, 'D MMM') : '',
            'Half Day_upper': date.getDate() !== last_date.getDate() ? date.getMonth() !== last_date.getMonth() ? _date_utils2.default.format(date, 'D MMM') : _date_utils2.default.format(date, 'D') : '',
            Day_upper: date.getMonth() !== last_date.getMonth() ? _date_utils2.default.format(date, 'MMMM') : '',
            Week_upper: date.getMonth() !== last_date.getMonth() ? _date_utils2.default.format(date, 'MMMM') : '',
            Month_upper: date.getFullYear() !== last_date.getFullYear() ? _date_utils2.default.format(date, 'YYYY') : ''
        };
    ...
    make_bars() {
        this.bars = this.tasks.map(task => {
            const bar = new _bar2.default(this, task);
    ...
    make_arrows() {
        ...
        for (let task of this.tasks) {
            ...
            arrows = task.dependencies.map(task_id => {
                ...
                const arrow = new _arrow2.default(this, this.bars[dependency._index], // from_task
    ...
    set_scroll_position() {
        ...
        const hours_before_first_task = _date_utils2.default.diff(this.get_oldest_starting_date(), this.gantt_start, 'hour');
    ...
    bind_grid_click() {
        _svg_utils.$.on(this.$svg, 'click', '.grid-row, .grid-header', () => {
            this.unselect_all();
            this.hide_popup();
        });
    }
    ...
    bind_bar_events() {
        ...
        _svg_utils.$.on(this.$svg, 'mousedown', '.bar-wrapper, .handle', (e, element) => {
            const bar_wrapper = _svg_utils.$.closest('.bar-wrapper', element);
        ...
        _svg_utils.$.on(this.$svg, 'mousemove', e => {
        ...
        _svg_utils.$.on(this.$svg, 'mouseup', e => {
            this.bar_being_dragged = null;
            bars.forEach(bar => {
                const $bar = bar.$bar;
    ...
    bind_bar_progress() {
        ...
        _svg_utils.$.on(this.$svg, 'mousedown', '.handle.progress', (e, handle) => {
            is_resizing = true;
            x_on_start = e.offsetX;
            y_on_start = e.offsetY;

            const $bar_wrapper = _svg_utils.$.closest('.bar-wrapper', handle);
        ...
        _svg_utils.$.on(this.$svg, 'mouseup', () => {
            is_resizing = false;
    ...
    show_popup(options) {
        if (!this.popup) {
            this.popup = new _popup2.default(this.popup_wrapper);
        }
...
},{"./date_utils":16,"./svg_utils":17,"./bar":18,"./arrow":19,"./popup":20,"./gantt.scss":28}],274:[function(require,module,exports) {
'use strict';
...
var gantt = new _frappeGantt2.default("#gantt", tasks, {
  header_height: 50,
  column_width: 30,
  step: 24,
  view_modes: ['Quarter Day', 'Half Day', 'Day', 'Week', 'Month'],
...
},{"frappe-gantt":12}],13:[function(require,module,exports) {
...
//# sourceMappingURL=/main.2f0b6898.map